...
- **走行中**: 9×4 `indep` の段2/段3 → 9×5 `sym` の 3 段(`run_chain.sh` が直列実行)。
  ログ = `playground_nozomi/km/runs/_logs/`。集計は
  `julia --project=. playground_nozomi/km/scripts/km_analyze.jl`
- [ ] **U スキャン**: ν=1/3 の残差が「U が理想 V₁ でないこと」由来かを決着させる
